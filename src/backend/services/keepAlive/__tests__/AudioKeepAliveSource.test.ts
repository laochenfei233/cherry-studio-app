...
const sourceModule =
  ...
  require('../AudioKeepAliveSource') as typeof import('../AudioKeepAliveSource');
const { AudioKeepAliveSource } = sourceModule;

describe('AudioKeepAliveSource', () => {
  ...
  test('shares one audio session across holders and stops on the last release', async () => {
    ...
    const second = source.acquire('jobs');
  ...
  test('stops audio on stop and no-ops later acquires', async () => {
    ...
    const lease = source.acquire('late');
    lease.release();
    await flushOperations();
    expect(mockSetAudioModeAsync).toHaveBeenCalledTimes(1);
  });
...
async function flushOperations() {
  // AudioKeepAliveSource loads expo-audio only on first use so importing the
