...
export const encodeCursor = (key: string | number, id: string): string =>
  `${encodeURIComponent(String(key))}:${id}`;
...
export function keysetOrdering(
  ...
  return {
    where: (cursor) =>
      or(
        after(keyCol, dir.major, cursor.key),
        and(eq(keyCol, cursor.key), after(idCol, dir.tie, cursor.id)),
      )!,
