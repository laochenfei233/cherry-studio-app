...
export function MessagePartSource({
  ...
  const containerClassName = isCard
    ...
    : 'min-h-10 flex-row items-center gap-2 rounded-md px-2 py-1.5 active:bg-secondary-active active:opacity-80';
  const iconClassName = isCard
    ? 'size-4 shrink-0 text-foreground'
    : 'size-4 shrink-0 text-muted-foreground';

  return (
    <BackgroundPressExclusion className={isCard ? undefined : '-mx-2'}>
      <Pressable
        {...props}
