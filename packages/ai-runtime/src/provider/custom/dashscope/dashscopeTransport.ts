...
function responseFamilyFor(descriptor: DashScopeModelDescriptor): ResponseFamily {
  ...
  if (
    path.endsWith('/image-generation/generation') ||
    path.endsWith('/multimodal-generation/generation')
  ) {
    return 'choices';
  }
...
class DashScopeTransport implements ImageGenerationTransport {
  ...
  async submit(
    ...
    const extraHeaders: Record<string, string> = descriptor.isSync
      ? {}
    ...
    while (attempts < maxAttempts) {
      ...
      try {
        ...
        if (status === 'FAILED' || status === 'CANCELED' || status === 'UNKNOWN') {
          throw new DashScopeTaskFailedError(
            result.output.message || `DashScope task ${status.toLowerCase()}`,
          );
        }
      } catch (error) {
        ...
        if (error instanceof DashScopeApiError && isTerminalHttpStatus(error.statusCode))
          throw error;
