...
import { formatApiHost, withoutTrailingApiVersion } from '../../../utils/provider';
...
function resolveNativeImageFamily(modelId: string): DmxapiNativeImageFamily {
  return (
    NATIVE_IMAGE_FAMILY_TABLE.find((entry) => entry.match(modelId))?.family ?? 'openai-compat-image'
  );
}
...
export function dmxapiUsesCustomTransport(modelId: string): boolean {
  return (
    resolveNativeImageFamily(modelId) === 'openai-compat-image' &&
    resolveDmxapiFamily(modelId) !== 'openai-flat'
  );
}
...
export function buildDmxapiTransport(settings: DmxapiProviderSettings): ImageGenerationTransport {
  const chatBaseURL =
    settings.endpointBaseURLs?.[ENDPOINT_TYPE.OPENAI_CHAT_COMPLETIONS] ?? settings.baseURL;
  if (!chatBaseURL) {
...
export function createDmxapiProvider(settings: DmxapiProviderSettings = {}): DmxapiProvider {
  ...
  const compatUrl = ({ path }: { path: string; modelId: string }) =>
  ...
  const anthropicBaseURL =
    ...
    formatApiHost(nativeBaseURL, true);
  const geminiBaseURL =
    settings.endpointBaseURLs?.[ENDPOINT_TYPE.GOOGLE_GENERATE_CONTENT] ??
    formatApiHost(nativeBaseURL, true, 'v1beta');
  const openaiBaseURL =
